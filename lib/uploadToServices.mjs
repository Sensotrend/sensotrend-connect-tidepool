...
export default function uploadToService() {
  ...
  uploadToService.uploadRecordsAndUpdateResponse = async function (
        ...
        for (const record of records) {
          ...
          try {
            ...
            if (singleUploadResults.errors === 0) {
              ...
              res.write(
                (uploadResults.records.length > 0 ? ',' : '') +
                JSON.stringify({ e: uploadResults.error })
              );
            }
    ...
    Promise.all(promises)
      ...
      .then(() => {
        ...
        res.write('],');
        res.write(`"count": ${uploadResults.records.length},`);
        res.write(`"created": ${uploadResults.created},`);
        res.write(`"skipped": ${uploadResults.skipped},`);
        res.write(`"errors": ${uploadResults.errors},`);
        if (uploadResults.batchResults) {
          res.write(`"results": ${JSON.stringify(uploadResults.batchResults)},`);
        }
        res.write('"success": 1}');
        res.end();
      });
  ...
  uploadToService.uploadBatch = function (FHIRClient, chunk, uploadResults, res, i) {
    return FHIRClient.uploadBatch(chunk)
      ...
      .catch((error) => {
        const errorString = JSON.stringify({ chunk, error });
        logger.error('Failed sending batch chunk ' + i + ': ' + errorString);
        res.write(
          (uploadResults.records.length > 0 ? ',' : '') + errorString
        );
        return uploadResults;
      });
  };
