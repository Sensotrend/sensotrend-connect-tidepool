...
export default function uploadToService() {
  ...
  uploadToService.uploadRecordsAndUpdateResponse = async function (
    ...
    const records = await DataFormatConverter.convert(data.data || data, converterOptions);
      ...
      for (const record of records) {
        ...
        try {
          ...
          if (singleUploadResults.errors === 0) {
            ...
            uploadResults.records.push(null); // just to simplify comma handling for the test
    ...
    Promise.all(promises)
       ...
       */
        res.write('"errors": 0,');
        try {
          const newRecords = simpleDataFormatConverter.convert(data, converterOptions);
          uploadResults.newRecords = newRecords;
        } catch(error) {
          logger.warn('Failed processing new records:\n' + JSON.stringify(error));
          uploadResults.newRecords = JSON.stringify(error);
        }
        res.write(`"newRecords": ${JSON.stringify(uploadResults.newRecords)},`);
