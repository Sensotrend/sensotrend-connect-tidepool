...
import authTools from './autTools.mjs';
...
export default function uploadToService() {
  ...
  uploadToService.uploadRecordsAndUpdateResponse = async function (
    ...
    for (let i = 0; i < records.length / chunkSize; i += 1) {
      const checkNotCacheToken = createSignatureAndRefreshToken(serverUserInfo);

      if (checkNotCacheToken.refresh) {
        const tokenData = checkNotCacheToken.tokenData;
        const subUser = jwt.decode(jwt.tokenData.accessToken);
