...
const batchSupported = (process.env.BATCHSUPPORTED && process.env.BATCHSUPPORTED.toLowerCase() === 'true'); // TODO: check from capability statement! Or at least from ENV.

export default function uploadToService(){
   
  //{email:, password: , generoitu oma koodi}
  uploadToService.uploadRecordsAndUpdateResponse = async function(serverUserInfo, data, res, env) {
    ...
    for (let i = 0; i < records.length / chunkSize; i += 1) {
       ...
       if (records.length > 1 && batchSupported) {
          promises.push(
            uploadToService.uploadBatch(FHIRClient,chunk,uploadResults)
          );
       } else {
...
uploadToService.fhirUpload = async function(FHIRClient, record, singleUploadResults){
 ...
 uploadToService.uploadBatch = function(FHIRClient,chunk,uploadResults ){
