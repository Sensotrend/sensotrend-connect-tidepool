...
function TidepoolRESTPlugin (models,env) {
   ...
   async function sessionValidationRoute (req, res, next) {
      ...
      if( req.session.serverUserInfo ){
         ...
         userInfo =  models.TidepoolTokenHandler.findOne({
      ...
      if (userInfo) {
         ...
         try{
            const uuid = uuidv4();
            const signature = createSignature(`Bearer ${userInfo.refresToken} || ${uuid}`).toString('base64');
            const user = await refreshToken(userInfo,signature, uuid);
            ...
            if(user.accessToken !== userInfo.accessToken ){
              
              

              const tokenData = models.TidepoolTokenHandler.findOne({
                  userId:  req.params.userId
               });

               tokenData.access_token = user.accessToken;
               tokenData.refresh_token = user.refresToken;
               tokenData.token_expiry_date = user.expires;
               tokenData.save();
            }
            ...
            logger.info(`Uploader user is not validatet ${e}`);
            return res.json({
               success: false,
         ...
         logger.info(`JWT validation error: ${JSON.stringify(req.headers)}`);
   ...
   // Note Libre does the batched dataset uploads
      ...
      if( serverUserInfo){
        ...
        logger.info('User information missing');
...
async function saveSetTempStorage(serverUserInfo, dataset, reqBody){
   ...
   const dataSetTempStorage = new models.DatesetTempStorage({
      ...
      refresh_token: serverUserInfo.refresToken,
   ...
   app.postAsync('/auth/login', async function (req, res) {
     ...
     try{
        ...
        logger.info(`Signature length: ${signatureBytes.length}`)
         ...
         const user = await axios.post(`${process.env.TOKEN_API_SERVER_ADDRESS}`,
         {
            signature: signature,
            nonce: uuid
         },
         ...
         const accessTokenDecode = jwt.decode(user.data.accessToken);
         ...
         if (user.data) {
            ...
            if(!foundTokenHandler){
               tidepoolTokenHandler = await models.TidepoolTokenHandler.create({
                  access_token: user.data.accessToken,
                  refresh_token: user.data.refresToken,
         ...
         if (response.authentication == 'success') {
           ...
           if( !tidepoolTokenHandler){
            ...
            req.session.serverUserInfo = Object.assign({email: credentials.name }, user.data);
            const r = {
               'emailVerified': true,
               'emails': [credentials.name],
  ...
  async function refreshToken(userInfo, signature, uuid){
      ...
      if( thisDate >= expiredDate){
         ...
         const userData = await axios.post(`${process.env.TOKEN_API_SERVER_ADDRESS}`,
         {
            signature: signature,
            nonce: uuid,
            refresh: userInfo.refresToken
         },
         {
            responseType: 'arraybuffer',
            headers: {
               'Authorization': `Bearer ${userInfo.refresToken}`
            }
        } );
        logger.info('Token is refreshed');
        req.session.serverUserInfo = userData;
        return userData;
      }
   ...
   */
   ...
   app.getAsync('/metadata/:userId/profile',profileCreateMiddleware, async function (req, res) {
      ...
      const user = jwt.decode(_tokenHandler.access_token);
   ...
   */
   ...
   app.post('/auth/logout', (req, res) => {
      ...
      if( req.headers[SESSION_TOKEN_HEADER] ){
         const deleteTidepoolHandler = await models.TidepoolTokenHandler.deleteOne({
            accessToken:  req.headers[SESSION_TOKEN_HEADER]
         });

         const {iat, exp} = jwt.decode(deleteTidepoolHandler.access_token);

         logger.info(`Deleted tidepoll iat: ${iat} exp: ${exp} `)
      }
      res.send({
         success: 1
      });
