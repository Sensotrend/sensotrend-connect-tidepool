...
import getNpmVersion from '../version/version.mjs';
...
export const batchSupported =
  process.env.BATCHSUPPORTED && process.env.BATCHSUPPORTED.toLowerCase() === 'true'; // TO2DO: check from capability statement! Or at least from ENV.
...
function TidepoolRESTPlugin(models, env) {
  ...
  async function getUserInfo(req) {
    try {
      const findUserData = await models.TidepoolTokenHandler.findOne({
        userId: req.params.userId,
      });

      if (findUserData === null && req.session.serverUserInfo === undefined) {
        return false;
      }

      return req.session.serverUserInfo
        ? req.session.serverUserInfo
        : {
            refreshToken: findUserData.refresh_token,
            accessToken: findUserData.access_token,
            expires: findUserData.token_expiry_date,
          };
    } catch (err) {
  ...
  async function getUserSignature(req, userInfo) {
    try {
      const uuid = uuidv4();
      const signature = authTools
        .createSignature(`Bearer ${userInfo.refreshToken} || ${uuid}`)
        .toString('base64');
      return await authTools.refreshToken(req, userInfo, signature, uuid);
    } catch (error) {
      throw new Error(JSON.stringify(error));
    }
  }

  async function findTokenData(req, userInfo) {
    try {
      return await models.TidepoolTokenHandler.findOneAndRemove({
        $or: [
  ...
  async function saveNewUserTidepoolToken(user, tokenData) {
    try {
      await models.TidepoolTokenHandler.create({
        ...
        name: user.name ? user.name : tokenData.name,
      });
    } catch (error) {
      throw new Error(JSON.stringify(error));
    }
  ...
  async function sessionValidationRoute(req, res, next) {
    ...
    try {
      ...
      if (userInfo) {
        ...
        try {
          const user = await getUserSignature(req, userInfo);
          ...
          const tokenData = await findTokenData(req, userInfo);

          await saveNewUserTidepoolToken(user, tokenData);
          ...
          logger.error(`Uploader user is not validated ${JSON.stringify(e)}`);
          return res.json({
            success: false,
      ...
      logger.error(`Error when get userInformation. Error is ${JSON.stringify(error)}`);
      return res.status(403).send({
        success: false,
  ...
  //
  // Missing / TO2DO: the client seems to have detection for already uploaded datasets, need to check how that works
  //
  ...
  // TO2DO read client code to find out what's expected
  ...
  app.get('/info', function (req, res) {
    logger.info('Call sensotrend-tidepool version');
    const infoText = `<h1>Sensotrend-connect-tidepool <br /> Version info ${getNpmVersion()}</h1>`;
      ...
      if (user.data) {
        ...
        response = {
          data: {
            ...
            server: env.FHIRServer, // TO2DO REMOVE
          },
  ...
  app.getAsync('/auth/login', sessionValidationRoute, async function (req, res) {
    ...
    if (userData) {
      ...
      logger.error('Authentication failed (login, invalid token)!\n' + JSON.stringify(userData));
      res.status(401).json({
  ...
  app.putAsync(
    ...
    async function (req, res) {
      ...
      if (_profile) {
        ...
        _profile.targetTimezone = req.body.patient.targetTimezone,
        await _profile.save();
      } else {
  ...
  function metricsResponse(req, res) {
    logger.info('/metrics/');
    if (logEnabled()) {
      logger.info('BODY: ' + JSON.stringify(req.body));
      logger.info('PARAMS: ' + JSON.stringify(req.params));
  ...
  app.post('/metrics/', metricsResponse);
  ...
  app.post('/auth/logout', async (req, res) => {
    try {
      ...
      if (req.headers[SESSION_TOKEN_HEADER]) {
        ...
        const sessionExpressId = req.sessionID;
        logger.info('Delete express-session in mongodb');
        req.session.destroy((err) => {
          if (err) {
            logger.error(`Error when destroy session. Error message: ${JSON.stringify(err)}`);
          } else {
            logger.info(`Session is now destroyed. Session id was ${sessionExpressId}`);
          }
        });
      }
    } catch (err) {
  ...
  function getTransferKey(fileAddress) {
    try {
      return JSON.parse(fs.readFileSync(fileAddress, 'utf8').trim());
    } catch (error) {
      throw new Error(error.message);
    }
  }

  async function verifySignature(jwToken) {
    try {
      ...
      const { payload, protectedHeader } = await jwtVerify(jwToken, publicKey);
  ...
  async function transferApi(userData, res) {
    ...
    try {
      ...
      const jwToken = new SignJWT({
        sub: userData.user_id,
        name: userData.name,
        email: userData.email,
      });
      jwToken.setProtectedHeader({ alg: 'ES256' });
      jwToken.setIssuedAt();
      jwToken.setIssuer('sensotrend-connect');
      jwToken.setAudience('sensotrend-api');
      jwToken.setExpirationTime('1m');
      const jwtAnswer = await jwToken.sign(privateKey);
      ...
      const jwe = new CompactEncrypt(encoder.encode(jwtAnswer));
      jwe.setProtectedHeader({
        alg: 'ECDH-ES+A128KW',
        enc: 'A128CBC-HS256',
        cty: 'jwt',
      });
      const encrypt = await jwe.encrypt(publicKey);

      logger.info(`Encrypt: ${encrypt}`);

      res.status(303).send(encrypt);
    } catch (error) {
      logger.error(`Error when creating session transfer token: ${JSON.stringify(error)}`);
      throw new Error(`Error when creating session transfer token: ${JSON.stringify(error)}`);
    }
  }

  root.postAsync('/user/information/verify', async (req, res) => {
    const userAccessInformation = req.data;

    if (verifySignature(userAccessInformation.jwt)) {
      try {
        const tokenData = await models.TidepoolTokenHandler.findOne({
          userId: userAccessInformation.userId,
        });
  ...
  root.getAsync('/patients', checkAuthorizationHeader, async (req, res) => {
    ...
    try {
      const transferPrivateKey = getTransferKey(process.env.TRANSFER_PRIVATE_KEY);
      const transferPublicKey = getTransferKey(process.env.TRANSFER_PUBLIC_KEY);
      const privateKey = await parseJwk(transferPrivateKey, 'ECDH-ES+A128KW');
      const { plaintext } = await compactDecrypt(getAuthorization, privateKey);

      const decoder = new TextDecoder();
      const decodeText = decoder.decode(plaintext);

      const publicKey = await parseJwk(transferPublicKey, 'ECDH-ES+A128KW');
      const verify = await compactVerify(decodeText, publicKey);
      const verifyDecode = decoder.decode(verify.payload);
      const verifyUserId = JSON.parse(verifyDecode).sub;
      const userData = await models.TidepoolTokenHandler.findOne({ userId: verifyUserId });

      if (!userData) {
        logger.error(`UserData is not found`);
        return res.send(404).send('<h1>You are not login to uploader</h1>');
      }
      ...
      logger.error(`Error when verify: ${JSON.stringify(error)}`);
      res.status(403).send({ error: 'Error when try to move user to dashboard' });
    }
  });
...
TidepoolRESTPlugin.version = getNpmVersion();
