...
function TidepoolRESTPlugin (models,env) {
 ...
 async function sessionValidationRoute(req, res, next) {
   ...
   if (userInfo) {
      ...
      try{
         const uuid = uuidv4();
         const signature = authTools.createSignature(`Bearer ${userInfo.refreshToken} || ${uuid}`).toString('base64');
...
async function saveSetTempStorage(serverUserInfo, dataset, reqBody){
  ...
  async function verifySignature(jwt) {
      ...
      try{
         ...
         logger.info(`Payload: ${JSON.stringify(payload)} and procetectedHeader: ${JSON.stringify(protectedHeader)}`);
