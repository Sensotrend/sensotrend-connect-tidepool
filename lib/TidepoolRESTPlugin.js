...
function TidepoolRESTPlugin (models,env) {
   ...
   async function sessionValidationRoute (req, res, next) {
      ...
      const userInfo = req.session.serverUserInfo;
      // decode token
      if (userInfo) {
...
async function saveSetTempStorage(serverUserInfo, dataset, reqBody){
   ...
   */
   ...
   function profileCreateMiddleware( req, res, next){
      const profile = {
         'fullName': encodeURIComponent(req.session.serverUserInfo.name),
         'name': encodeURIComponent(req.session.serverUserInfo.name),
         'email': encodeURIComponent(req.session.serverUserInfo.email),
         'patient': {
            'birthday': '1900-01-01',
