   ...
   app.postAsync('/auth/login', async function (req, res) {
     ...
     try{
        ...
        logger.info(`Signature length: ${signatureBytes.length}`)
         ...
         logger.info(`Tämä on ennen authentizatio ongelmaa: ${JSON.stringify(user)}`);
         ...
         if (response.authentication == 'success') {
            ...
            req.session.serverUserInfo = user.data;
            const r = {
               'emailVerified': true,
               'emails': ['foo@bar.com'],
