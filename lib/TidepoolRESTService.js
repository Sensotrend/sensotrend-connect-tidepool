...
function TidepoolRESTService (env) {
   ...
   async function sessionValidationRoute (req, res, next) {
      ...
      if (token) {
         ...
         try{
            const uuid = uuidv4();
            const signature = createSignature(`Bearer ${token} || ${uuid}`);
            const user = await refreshToken(req,signature, uuid);
...
async function saveSetTempStorage(serverUserInfo, dataset, reqBody){
   ...
   app.postAsync('/auth/login', async function (req, res) {
     ...
     try{
        ...
        const uuid = uuidv4();
         const signature = createSignature(`Basic ${token} || ${uuid}`);

         const user = await axios.get(`${process.env.TOKEN_API_SERVER_ADDRESS}?nonce=${encodeURI(uuid)}&signature=${encodeURI(signature)}`,{
            headers: {
  ...
  async function refreshToken(req, signature, uuid){
      ...
      if( thisDate >= expiredDate){
         ...
         const userData = await axios.get(`${process.env.TOKEN_API_SERVER_ADDRESS}?nonce=${encodeURI(uuid)}&signature=${encodeURI(signature)}`,{
            headers: {
