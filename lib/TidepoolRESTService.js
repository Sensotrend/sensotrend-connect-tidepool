...
export const batchSupported = (process.env.BATCHSUPPORTED.toLowerCase() === 'true');; // TODO: check from capability statement! Or at least from ENV.
...
function TidepoolRESTService (env) {
   ...
   async function sessionValidationRoute (req, res, next) {
      ...
      //Tokenin hallinta muutetaan tännekkin, erilaikseksi.
      ...
      if (token) {
         ...
         const private_key = fs.readFileSync('../sensotrend-connect-tidepool/keys/privaKey.pem','utf8');

         const signer = crypto.createSign('RSA-SHA256');
         signer.update(`Basic ${token} || ${uuidv4()} `);
         signer.end;
         ...
         try{
            env.logger.info(`refreshToken: ${userData.refresToken}`);
            const user = await axios.get(`${process.env.TOKEN_API_SERVER_ADDRESS}`,{
                headers: {
                  'Authorization': `Bearer ${userData.refresToken}`
                },
            } );

            env.logger.info(JSON.stringify(user.data));

            if(user.data){

                req.session.serverUserInfo = user.data;
                const decoded = jwt.decode(user.data.accessToken);
   ...
   async function uploadRecordsAndUpdateResponse(userId, data, res) {
      ...
      // Tehdään tähän uusi tokenin hakeminen.
      //********************************************************* */
...
async function saveSetTempStorage(dataset, reqBody){
   ...
   const dataSetTempStorage = new DatesetTempStorage({
      access_token: reqBody.token,
      refresh_token: reqBody.refresh_token,
      token_expiry_date: reqBody.expiry_date,
