...
function FIPHR(env) {
  ...
  const client = new fiphrIssuer.Client(
    tokenUrl.host.indexOf('sandbox') > 0 ? sandboxOpts : productionOpts
  ); // => Client
  ...
  app.get('/callback', async (req, res) => {
    ...
    try {
      const token = await client.oauthCallback(OauthCallbackURL, req.query, {
  ...
  app.get('/config', async (req, res) => {
    ...
    const config = email
      ? {
          ...
          email,
          secret: user.site_secret,
          development: user.contact_development_flag,
          notifications: user.contact_notifications_flag,
        }
