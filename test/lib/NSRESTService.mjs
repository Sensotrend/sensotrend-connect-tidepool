...
import awaitjsModule from '@awaitjs/express';
import basicAuthParser from 'basic-auth';
import EntryProcessor from './nshandlers/entries.mjs';
import TreatmentProcessor from './nshandlers/treatments.mjs';

const { decorateApp } = awaitjsModule;
...
function NSRestServer(env) {
  ...
  NightscoutRESTServer.getAsync(
    '/treatments*',
    loadUserIntoSessionOrFail,
    async (req, res) => {
      ...
      const treatments = await Treatments.getTreatments(
        env.FHIRServer,
        req.user.sub,
        token,
        req.query
  ...
  async function putOrPostTreatment(req, res) {
    ...
    const token = await env.oauthProvider.getAccessTokenForUser(req.user);
    const treatments = await Treatments.postTreatments(
      env.FHIRServer,
      req.user.sub,
      token,
      data
  ...
  NightscoutRESTServer.postAsync(
    '/treatments*',
    loadUserIntoSessionOrFail,
    async (req, res) => {
  ...
  NightscoutRESTServer.postAsync(
    ...
    async (req, res) => {
      ...
      // TODO store the data
      logger.info(
        'Got device status for user ' + req.user.user_id + ':\n' + JSON.stringify(req.body)
      );
      res.send(req.body);
    }
  );
